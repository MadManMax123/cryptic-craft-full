...
export default async function handler(req, res) {
  try {
    ...
    const flag = process.env.LEVEL8_FLAG || "";
    ...
    if (!flag) {
      return res.status(500).json({ error: "Missing LEVEL8_FLAG" });
    }

    const cookies = parseCookies(req.headers.cookie || "");
    const token = cookies.level8_auth;
    ...
    console.error("get-level8-flag error:", err);
